...
def plot_metrics(metrics, disease_cols, save_path="../outputs/Fine_Tuned/", selected_diseases=None):
    ...
    for metric_name, values in metrics_to_plot.items():
        ...
        plt.title(f'{metric_name} per Disease Class Fine Tuned')
        plt.legend()
        plt.tight_layout()
    ...
    results_df.to_csv(os.path.join(save_path, 'fine_tuned_disease_metrics.csv'), index=False)
    ...
    pd.DataFrame(macro_metrics).to_csv(os.path.join(save_path, 'fine_tuned_macro_metrics.csv'), index=False)
    
    print(f"✅ Evaluation results saved to {save_path}")

def confusion_matrix_per_class(y_true, y_pred, disease_cols, save_path="../outputs/Fine_Tuned/", selected_diseases=None):
    ...
    for i in selected_indices:
        ...
        plt.title(f'Fine Tuned Confusion Matrix: {disease}')
        plt.colorbar()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(save_path, f'fine_tuned_confusion_matrix_{disease}.png'))
        plt.close()
    
    pd.DataFrame(confusion_data).to_csv(os.path.join(save_path, 'fine_tuned_confusion_matrices.csv'), index=False)

def plot_selected_diseases_metrics(metrics, disease_cols, y_true, y_probas, selected_diseases, save_path="../outputs/Fine_Tuned/"):
    ...
    plt.title('Fine Tuned ROC Curves for Selected Diseases')
    plt.legend(loc="lower right")
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'fine_tuned_selected_diseases_roc_curves.png'))
    ...
    selected_df.to_csv(os.path.join(save_path, 'fine_tuned_selected_diseases_metrics.csv'), index=False)
    ...
    table.scale(1.2, 1.5)
    plt.title('Fine Tuned Selected Disease Categories Performance Metrics')
    plt.tight_layout()
    plt.savefig(os.path.join(save_path, 'fine_tuned_selected_diseases_table.png'))
...
def main():
    ...
    model_path = "../src/fine_tuned_model.pth"
    threshold = 0.125
